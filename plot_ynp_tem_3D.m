...
%first, check to see what "block" you are in by interrogating the flight
%line on google earth:
filein = ('C:\Users\aparseki\Dropbox\YNP\USGS_YNP_SkyTEM\AirborneElectro\YellowstoneNP_2016_MudPot_ProcessedData_Models.csv'); %input path to USGS csv file here
...
% choose point of interest in GoogleEarth using UTM units (easting | northing)
%poi = [520089 4947287]; % Beryl Spring
poi = [544982.97 4941822.27]; % Sulphur Cauldron 
...
%SF = [544859.88 4941160.29 545035.86 4942242.38]; %[sulphur cauldron north/south]
SF = [543751 4942127.6 546768.11  4941040.8]; %[sulphur cauldron east-west]
%SF = [518483 4947529 521568 4947534]; %[Beryl] % start and end point of a line, UTM meters
...
radius = 50; %how far around the node to look for other points
sitecnt = 0; % just a counter
...
%figure
%scatter3(linD(:,2),linD(:,3),linD(:,4),ones(length(linD),1).*50,log10(linD(:,5)),'filled'); hold on % show the line
...
for i = 1:length(linD)
    for j = 1:29 %location of all inverted RHO values
        pos = [0 linD(i,2) linD(i,4)-linD(i,j+64)  linD(i,4)-linD(i,j+94)]; % the position of each pixel (2 = easting, 4=ground elevation, 64 is start of tops, 94 is start of bottoms
...
%% Plot 3D resistivity section

figure
subplot(4,4,1:12)
%3dplot = [linD(:,2) linD(:,3) mean([])]
%scatter3(a,b,c,10,d,'filled')
wd = 40; %plotting width
cntr = 1;
for i = 1:length(linD)
    for j = 1:29 %location of all inverted RHO values
        if linD(i,j+94)<linD(i,212) % avoid voxels below the DOI
            ZZ = mean([(linD(i,4)-linD(i,j+64)) (linD(i,4)-linD(i,j+94))]); %just calculates the middle of the depth grid for each point
            tdp(cntr,:) = [0 linD(i,2) linD(i,3) ZZ log10(linD(i,j+4)) linD(i,212)]; % the position of each pixel (2 = eastibng, 3= northing 4=ground elevation, 64 is start of tops, 94 is start of bottoms
            cntr = cntr+1;
        end
    end
end

scatter3(tdp(:,2),tdp(:,3),tdp(:,4),wd,tdp(:,5),'filled','square'); hold on %plot the 3D data
scatter3(544973.12, 4941808.35, 2355,50,'red','filled'); % location of Sulphur Caludron
